...
export default function TablesPage() {
  const { tables, orders, error: tablesError } = useTables();
  const { menuItems, error: menuError } = useMenu();
  const [selectedOrderId, setSelectedOrderId] = useState<string | null>(null);
  const [selectedTableId, setSelectedTableId] = useState<string | null>(null);
  const [panelOpen, setPanelOpen] = useState(false);
  const lastErrorRef = useRef<string | null>(null);
  const selectedTableIdRef = useRef<string | null>(null);
  const { order: currentOrder, error: orderError } = useOrder(selectedOrderId);
  const {
  ...
  const totalError = tableActionError ?? orderActionError ?? orderError ?? tablesError ?? menuError;
