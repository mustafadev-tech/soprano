...
async function listClosedOrderSummaries(window: ReportWindow): Promise<DailySummaryRow[]> {
...
async function listOrderItemDetails(orderIds: string[]): Promise<OrderItemDetailsViewRow[]> {
  if (!orderIds.length) {
    return [];
  }
...
async function countOpenOrdersAtEndOfDay(window: ReportWindow): Promise<number> {
...
async function getSnapshotForDate(date: string): Promise<SnapshotRow | null> {
...
async function listSnapshotRowsInWindow(endDate: string): Promise<SnapshotRow[]> {
...
export async function refreshDailyReportSnapshot(dateParam: string | null): Promise<void> {
  const window = resolveReportWindow(dateParam);
  const [closedOrderRows, openOrdersCount] = await Promise.all([
    listClosedOrderSummaries(window),
    countOpenOrdersAtEndOfDay(window),
  ]);

  const orderIds = closedOrderRows.map((row) => row.order_id);
  const itemRows = await listOrderItemDetails(orderIds);
  ...
  await upsertReportSnapshot(report);
}
...
export async function getDailyReport(dateParam: string | null): Promise<DailyReportSummary> {
  const window = resolveReportWindow(dateParam);
  const [snapshotForDate, recentSnapshotRows, closedOrderRows, openOrdersCount] = await Promise.all([
    getSnapshotForDate(window.date),
    listSnapshotRowsInWindow(window.date),
    listClosedOrderSummaries(window),
    countOpenOrdersAtEndOfDay(window),
  ]);
  ...
  if (hasLiveData) {
    ...
    await upsertReportSnapshot(liveReport, snapshotUpdatedAt);
